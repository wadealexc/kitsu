<script lang="ts">
    ...
    export let autocomplete: AutoFill = 'off';
...
<div class="{outerClassName} {className}">
    <label class="sr-only" for={id}>{placeholder || 'Password'}</label>
    <input
        {id}
        class={`${inputClassName} ${show ? '' : 'password'} outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-600`}
        {placeholder}
